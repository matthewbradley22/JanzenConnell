...
popSize = NULL
avgDens = NULL
parameterTests = tibble("dens1" = NULL, "dens2" = NULL, "popSize" = NULL, "avgDens" = NULL)
### MAIN LOOP ####

##Currently using for loops to determine parameters##

for (i in seq(2, 6, by = 0.2)){
  for(j in seq(20, 320, by = 30)){
    data <-  tibble("ID" = 1:numInd, "age" = sample(150, 50, replace = TRUE), 
                    species = sample(numSpecies, 50, replace = TRUE), "xlocation" = runif(50, 0, 100), 
                    "ylocation" = runif(50,0,100), "parentalDistance" = 0)
    DensParam =  i
    ...
    currentParams = tibble("dens1" = DensParam, "dens2" = DensParam1, "popSize" = mean(popSize), "avgDens" = mean(avgDens))
...
plot_ly(data = parameterTests, x = ~dens1, y = ~dens2, z = ~popSize, type="scatter3d", 
        mode="markers", color= ~popSize)
